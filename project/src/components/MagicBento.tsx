...
export interface CardData {
  ...
  colSpan?: number;           // New: Allow spanning multiple columns
  className?: string;         // New: Custom classes for specific cards
}
...
interface MagicBentoProps {
  ...
  gridClassName?: string; // New: Allow overriding the grid layout (e.g., 3 columns)
}
    ...
    <>
      ...
      <BentoCardGrid gridRef={gridRef} className={gridClassName}>
        {cards.map((card, index) => {
          ...
          // If the card has children (custom content), render that.
          if (card.children) {
              return (
                  <ParticleCard
                    key={card.id || index}
          ...
          // Default rendering for simple stat cards
          if (enableStars) {
            return (
              <ParticleCard
                key={card.id || index}
              ...
              >
                <div className="magic-bento-card__header">
                  <div className="magic-bento-card__label flex items-center">
                    {card.icon && <span className="mr-2 text-white/70">{card.icon}</span>}
                    {card.label}
                  </div>
                </div>
